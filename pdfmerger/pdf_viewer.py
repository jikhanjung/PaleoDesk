...
class PDFViewer(QWidget):
    ...
    def displayPages(self):
        ...
        for i, pixmap in enumerate(pixmaps):
            ...
            x_offset += pixmap.width() + 20  # 페이지 간격
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() == Qt.Key.Key_Left or event.key() == Qt.Key.Key_PageUp:
            ...
            # 확대
            new_value = min(self.zoom_slider.value() + 10, 200)
            self.zoom_slider.setValue(new_value)
        elif event.key() == Qt.Key.Key_Minus:
            # 축소
            new_value = max(self.zoom_slider.value() - 10, 25)
            self.zoom_slider.setValue(new_value)
    ...
    def addCropOverlay(self, x, y, width, height):
        ...
        # 상단 크롭 영역
        if self.top_crop > 0:
            top_rect = QGraphicsRectItem(x, y, width, self.top_crop * self.zoom_level)
        ...
        if self.bottom_crop > 0:
            bottom_y = y + height - (self.bottom_crop * self.zoom_level)
            bottom_rect = QGraphicsRectItem(x, bottom_y, width, self.bottom_crop * self.zoom_level)
