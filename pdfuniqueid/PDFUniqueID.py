...
from PDFModels import (db, PDFDocument) 
import hashlib
import uuid
import copy
import io
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.heatmap_view = ScrollableHeatmapWidget(self,None)
        self.content_scroll.setWidget(self.heatmap_view)
    ...
    def create_toolbar(self):
        ...
        # Analyze action
        analyze_action = QAction("Get Embedding", self)
        analyze_action.triggered.connect(lambda: self.embed_pdf(self.current_file_path, force_analysis=True) if self.current_file_path else None)
        ...
        batch_analyze_action = QAction("Batch Embedding", self)
        batch_analyze_action.triggered.connect(self.batch_embed)
    ...
    def embed_pdf(self, file_path, force_analysis=False):
        ...
        try:
            ...
            self.status_label.showMessage("Embedding PDF...", 0)  # 0 means show until changed
            QApplication.setOverrideCursor(QCursor(Qt.CursorShape.WaitCursor))
            
            try:
                # Get service URL from settings
                settings = QSettings(COMPANY_NAME, PROGRAM_NAME)
                base_url = settings.value("embedding_service/url", "").rstrip('/')
                if not base_url:
                    raise ValueError("Service URL not configured")
                
                logger.info(f"Using embedding service at: {base_url}")
